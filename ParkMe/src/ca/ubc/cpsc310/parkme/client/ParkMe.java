...
public class ParkMe implements EntryPoint {
	...
	private void getLocation(final ParkingLocation parkingLoc) {
		...
		if (parkingLoc.getStreet().equals("Vancouver")) {
			...
			LatLng latlong = LatLng.create(parkingLoc.getStartLat(), parkingLoc.getStartLong());
			...
			geocoder.geocode(request, new Geocoder.Callback() {
				...
				public void handle(JsArray<GeocoderResult> results, GeocoderStatus status) {
					...
					if (status == GeocoderStatus.OK) {
						...
						GeocoderAddressComponent addressComp = location.getAddressComponents().get(1);
						String street = addressComp.getLongName();
						parkingLoc.setStreet(street);
						loadDataService.setStreet(street, parkingLoc.getParkingID(), new AsyncCallback<Void>() {
			...
			System.out.println("i already have data");
			//displayParking(parkingLoc);
			return;
	...
	private void getAllLocations() {
		loadDataService.getUnknownStreets(new AsyncCallback<ParkingLocation[]>() {
